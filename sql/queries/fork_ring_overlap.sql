...
select keyimg, 'XMO' as fork, #xmo_out as ringsize, #(xmo_out & xmr_out) as overlap from xmo_rings join xmr_rings using(keyimg);
...
select keyimg, 'XMV' as fork, #xmv_out as ringsize, #(xmv_out & xmr_out) as overlap from xmv_rings join xmr_rings using(keyimg);
...
COPY
    (select case when ringsize<= 9 then ringsize else 10 end as ringsize, count(case when fork ='XMO' then 1 end) as XMO, count(case when fork ='XMV' then 1 end) as XMV from fork_ring_overlap group by 1 order by 1 asc)
